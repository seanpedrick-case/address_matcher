...
def load_matcher_data(in_text, in_file, in_ref, in_colnames, in_refcol, in_joincol, Matcher):
        ...
        Matcher.ref = pd.read_csv(in_ref.name, delimiter = ",", low_memory=False)#, encoding="utf-8")    
        ...
        return Matcher#Matcher.search_df, Matcher.search_df_key_field, Matcher.search_address_cols, Matcher.search_postcode_col,\
               #Matcher.in_colnames_list, Matcher.in_joincol_list, Matcher.ref, Matcher.ref_address_cols, Matcher.standard_llpg_format


# # DF preparation functions

# def filter_not_matched(match_results_output_not_std, search_df, search_df_key_field):
#     match_results_output_not_std_not_matched = match_results_output_not_std[match_results_output_not_std["full_match"] != True]
#
#     not_matched = search_df.reset_index()[search_df_key_field].astype(str).isin(match_results_output_not_std_not_matched[search_df_key_field])
#
#     search_df_not_matched = search_df[list(not_matched)]
#     
#     return search_df_not_matched
...
def filter_not_matched(
    ...
    return search_df[~matched]


# def prepare_search_address_string(search_df):
#     
#         search_df_out = pd.DataFrame(data={"full_address":[search_df]})
#         
#         search_df_out["postcode"] = extract_postcode(search_df_out, "full_address").dropna(axis=1)[0]
#         
#         search_df_out["full_address"] = remove_postcode(search_df_out, "full_address")
#                
#         # Set key field for joining back after the search is done to be index
#         search_df_key_field = "index"
#         
#         search_df_out = search_df_out.reset_index()
#         
#         search_address_cols = ["full_address"]
#         search_postcode_col = ["postcode"]
#         
#         return search_df_out, search_df_key_field, search_address_cols, search_postcode_col
...
# def prepare_search_address(search_df, search_address_cols, search_postcode_col, search_df_key_field):
#     
...
#     search_df[search_address_cols] = search_df[search_address_cols].apply(clean_col)          
#     
#     # Add in postcode
#     search_address_cols_pcode = search_address_cols.copy()
#     #search_address_cols_pcode.extend(search_postcode_col)
#     
#     full_address  = search_df[search_address_cols_pcode].apply(lambda row: ' '.join(row.values.astype(str)), axis=1) 
#     
#     search_df["full_address"] = full_address.str.replace("  "," ").str.replace("  "," ").str.strip()
#     
#     # Set key field for joining back after the search is done to be index
#     if ((search_df_key_field == "index") & ~("index" in search_df.columns)):        
#         search_df = search_df.reset_index()
#         
#     if isinstance(search_postcode_col, list):
#         search_postcode_col = search_postcode_col[0]
#         
#     search_df = search_df.rename(columns={search_postcode_col:"postcode"})
#     
#     search_df[search_df_key_field] = search_df[search_df_key_field].astype(str)
#     
#     print(search_df.columns)
#     
#     
#     
#     return search_df, search_df_key_field
...
def prepare_ref_address(ref, ref_address_cols, new_join_col = ['UPRN'], standard_cols = True):
    ...
    if standard_cols == True:
        ...
        ref_df["fulladdress"] = ref_df['SaoText'] +\
            " " + ref_df["SaoStartNumber"].astype(str) + ref_df["SaoStartSuffix"] + "-" + ref_df["SaoEndNumber"].astype(str) + ref_df["SaoEndSuffix"] + " " + ref_df["PaoText"] +\
             " " + ref_df["PaoStartNumber"].astype(str) + ref_df["PaoStartSuffix"] + "-" + ref_df["PaoEndNumber"].astype(str) + ref_df["PaoEndSuffix"] + " " + ref_df["Street"] +\
            " " + ref_df["PostTown"] +\
            " " + ref_df["Postcode"]
    ...
    ref_df.to_csv("ref_df_test.csv", index = None)
...
def standardise_wrapper_func(search_df:PandasDataFrame, ref_df:PandasDataFrame,\
                               standardise = False, filter_to_lambeth_pcodes = False, match_task = "fuzzy"):
    ...
    ### Filter addresses to match to lambeth only
...
def remove_flat_one_number_address (df:PandasDataFrame, col1:PandasSeries) -> PandasSeries:
    ...
    df['two_numbers_in_address'] =  df[col1].str.lower().str.contains(r"(?:\d+.*?)[^a-zA-Z0-9_].*?\d+")
    ...
    df['one_number_no_flat'] =  df[df['selected_rows'] == True][col1].str.replace(r"(\bapartment\b)|(\bapartments\b)", "", regex=True\
                                  ).str.replace(r"(\bflat\b)|(\bflats\b)", "", regex=True).str.replace(r"(\broom\b)|(\brooms\b)", "", regex=True)
...
# import pandas as pd
#
# example = pd.read_csv("address_parse_example.csv")
#
# out_df = remove_flat_one_number_address(example, "address")
#
# out_df
...
def extract_flat_no (df:PandasDataFrame, col1:PandasSeries) -> PandasSeries:
    ...
    '''  
    try:
        flat_no = df[df[col1].str.lower().str.replace(r"^\bflats\b","flat", regex=True).str.contains(\
              r"^\d+([a-z]|[A-Z])(?!.*\d+)|\bflat\b|\bapartment\b|(\d+.*?)[^a-zA-Z0-9_].*?\d+")][col1].str.replace(\
             "no.","", regex=True).str.extract(r'^\d+([a-z]|[A-Z])(?!.*\d+)|flat (\w+)|apartment (\w+)|(\d+.*?)[^a-zA-Z0-9_].*?\d+'\
                                                                          ).rename(columns = {0:"prop_number", 1:"flat_number",2:"apart_number",3:"first_sec_number"})
    except:
...
def standardise_address(df:PandasDataFrame, col:str, out_col:str, standardise:bool = True, out_london = False) -> PandasDataFrame:
   ...
   # "\\b(?:[A-Z][A-HJ-Y]?[0-9][0-9A-Z]? ?[0-9][A-Z]{2}|GIR ?0A{2})\\b$|(?:[A-Z][A-HJ-Y]?[0-9][0-9A-Z]? ?[0-9]{1}?)$|\\b(?:[A-Z][A-HJ-Y]?[0-9][0-9A-Z]?)\\b$",""\
    ...
    if standardise:
        ...
        df_copy[out_col] = merge_columns(df_copy, out_col, 'add_no_pcode_house_comma_no', 'floor_replacement')
    ...
    #try:    
    df[['prop_number','flat_number', 'apart_number','first_sec_number']] = extract_flat_no(df_copy, out_col)
    #except:
...
def refine_export_results (results_df:PandasDataFrame, 
                           ...
                           fuzzy_match_limit=75) -> PandasDataFrame:
    '''
    ...
    diag_shortlist["fuzzy_score_match"] = diag_shortlist['fuzzy_score'] >= fuzzy_match_limit #eyeballed it
    diag_shortlist = diag_shortlist.fillna("")


    '''
    Following creates a filter (mask) that matches together the derived room/property/flat numbers between
    the two lists (best matches only).
    '''
   
    mask_prop_only =\
    (diag_shortlist["reference_property_number"] == diag_shortlist["matched_property_number"])
    diag_shortlist["property_number_match"] = mask_prop_only
    
    mask_flat_only =\
    (diag_shortlist['matched_flat_number'] == diag_shortlist['reference_flat_number'])
    diag_shortlist["flat_number_match"] = mask_prop_only
    
    mask_room_only =\
    (diag_shortlist['matched_room_number'] == diag_shortlist['reference_room_number'])
    diag_shortlist["room_number_match"] = mask_prop_only
    
    mask = (diag_shortlist["reference_property_number"] == diag_shortlist["matched_property_number"]) & \
    (diag_shortlist["reference_flat_number"] == diag_shortlist["matched_flat_number"]) #& \
    ...
    diag_shortlist = diag_shortlist.rename(columns = {"search_orig_address":"search_orig_address",
                                                      "search_mod_address": "search_mod_address",
                                                      "matched_flat_number":"search_flat_number",
    ...
    if there is no number match, then property match, then best fuzzy score, then show all options
    '''  
    diag_shortlist = diag_shortlist.sort_values(["search_mod_address", 'full_number_match', "search_room_number", "fuzzy_score"], ascending = False)
    diag_best_match = diag_shortlist.drop_duplicates("search_mod_address")

    ## Split into matched and non-matched addresses for diagnostic purposes
    diag_shortlist_match = diag_best_match[["search_orig_address",'reference_orig_address', 
                                                                   'full_match',
                                          'full_number_match', 'room_number_match','flat_number_match',
                                            'property_number_match','fuzzy_score_match','fuzzy_score',
                                           "search_mod_address","reference_mod_address",'Postcode']] # ,'uprn']

    diag_best_match_matched = diag_shortlist_match[diag_shortlist_match['full_match'] == True].sort_values(\
                          ['fuzzy_score'], ascending = True)


    diag_best_match_not_matched = diag_shortlist_match[diag_shortlist_match['full_match'] == False].sort_values(\
                           ['fuzzy_score'], ascending = False)
    
    return compare_all_candidates, diag_shortlist, diag_best_match,\
           diag_best_match_matched, diag_best_match_not_matched
...
def create_match_summary(match_results_output, df_name):
        ...
        #print(records_attempted)

        dataset_length = len(match_results_output["full_match"])
        #dataset_length

        records_not_attempted = int(dataset_length - records_attempted)
        #print(records_not_attempted)

        match_rate = str(round((full_match_count / dataset_length) * 100,1))
        #match_rate

        match_fail_rate = str(round((match_fail_count / dataset_length) * 100,1))
        #match_fail_rate

        not_attempted_rate = str(round((records_not_attempted / dataset_length) * 100,1))
        #not_attempted_rate

        summary = ("For the " + df_name + " dataset, the fuzzy matching algorithm successfully matched " + full_match_count.astype(str) +\
                   " records, which is " + match_rate +\
           "% of the dataset. The algorithm did not attempt to match " + not_attempted_rate +  "% of the dataset. This leaves " + match_fail_count.astype(str) + " records left to match.")
...
def run_fuzzy_match(Matcher, standardise = False, nnet = False, file_stub= "not_std_", df_name = "Fuzzy not standardised"):
        ...
        if nnet == False:
            ...
            diag_shortlist,\
            diag_best_match,\
            diag_best_match_matches,\
            diag_best_match_not_matched,\
        ...
        #Matcher.search_df = Matcher.search_df_not_matched
      
        Matcher.match_outputs_name = "match_results_output_" + file_stub + Matcher.file_name + "_" + today_rev + ".csv"
        Matcher.results_orig_df_name = "results_on_orig_df_" + file_stub + Matcher.file_name + "_" + today_rev + ".csv"
...
def combine_two_matches(OrigMatchClass, NewMatchClass, df_name):
        ...
        NewMatchClass.results_on_orig_df = combine_std_df_remove_dups(OrigMatchClass.results_on_orig_df, NewMatchClass.results_on_orig_df, orig_addr_col = NewMatchClass.search_df_key_field, match_col = 'Matched with ref record')
        ...
        NewMatchClass.match_outputs_name = "match_results_output_std_" + NewMatchClass.file_name + "_" + today_rev + ".csv"
        NewMatchClass.results_orig_df_name = "results_on_orig_df_std_" + NewMatchClass.file_name + "_" + today_rev + ".csv"
...
def join_to_orig_df(match_results_output, search_df, search_df_key_field, new_join_col):
    ...
    if (search_df_key_field == "index"):
        match_results_output_success[search_df_key_field] = match_results_output_success[search_df_key_field].astype(int)
        search_df_j = search_df.drop("index", axis=1, errors='ignore').merge(match_results_output_success[ref_join_cols], left_index=True, right_on = search_df_key_field, how = "left")
    
    
    else:
        search_df_j = search_df.merge(match_results_output_success[ref_join_cols],how = "left", on = search_df_key_field)

    
    # Drop columns that aren't useful
    search_df_j = search_df_j.drop(["full_address_search","postcode_search", "full_address_1", "full_address_2"],\
    ...
    search_df_j = search_df_j.rename(columns={"reference_orig_address":"ref matched address",\
...
def string_match_by_post_code_multiple(match_col:PandasSeries, reference_col:PandasSeries,
    ...
    for pc_match, add_match in match_col.items(): # .iteritems() is deprecated, so replaced
        ...
        try:
            ...
            ### TRY ADDING TO VARIABLE INSTEAD OF TO RESULTS[ADD_MATCH] DIRECTLY, THEN PASS TO RESULTS[ADD_MATCH]
        ...
        #results[add_match] = results[add_match] + [current_index]
...
def full_fuzzy_match(search_df, ref, standardise, ref_address_cols,\
                     ...
                     fuzzy_match_limit, fuzzy_scorer_used, fuzzy_search_addr_limit = 20,
                    filter_to_lambeth_pcodes=True, new_join_col=["UPRN"]):
    ...
    search_df_prep_join, ref_join, search_df_prep_match_list, ref_df_match_list, search_df_stand_col, ref_df_stand_col =\
                                                standardise_wrapper_func(search_df_prep.copy(), ref_df, standardise = standardise)
    ...
    match_results_output, compare_all_candidates, diag_shortlist, diag_best_match,\
        diag_best_match_matches, diag_best_match_not_matched = _create_fuzzy_match_results_output(results, search_df_prep_join, ref_df, ref_join,\
                                                                                                 fuzzy_match_limit, search_df_prep, search_df_key_field, new_join_col, standardise)
    ...
    if sum(match_results_output['full_match']==False) == 0:
        ...
        return compare_all_candidates, diag_shortlist, diag_best_match,\
        diag_best_match_matches, diag_best_match_not_matched,\
    ...
    #print(search_df_prep_join_street['street'])
    ...
    match_results_output_st, compare_all_candidates_st, diag_shortlist_st, diag_best_match_st,\
    diag_best_match_matches_st, diag_best_match_not_matched_st = _create_fuzzy_match_results_output(results_st, search_df_prep_join_street, ref_df, ref_join,\
                                                                                                 fuzzy_match_limit, search_df_prep, search_df_key_field, new_join_col, standardise)
...
def _create_fuzzy_match_results_output(results, search_df_prep_join, ref_df, ref_join, fuzzy_match_limit, search_df_prep, search_df_key_field, new_join_col, standardise):
...
def text_to_model_input_local(in_text, model_type = "estimator"):
    addresses_out = []
    model_input_out = []
    encoded_text = []
...
def predict_serve_conv_local(in_text:List[str], predictions) -> List[Dict[str, str]]:
...
def full_predict_func(list_to_predict, model):
    
    if hasattr(model, "summary"): # Indicates this is a keras model rather than an estimator
        model_type = "keras"
    else: model_type = "estimator"
    
    addresses_out, model_input = text_to_model_input_local(list_to_predict, model_type) 
    ...
    output = predict_serve_conv_local(list(list_to_predict), predictions_list_reformat)
...
def score_based_match(predict_df_search, ref_search, orig_search_df, matching_variables,
    ...
    ref_search[matching_variables] = ref_search[matching_variables].astype(str) 
    ...
    if len(pairsSBM) == 0: return 0,0,0,0
    ...
    scoresSBM_search_m_j['full_match_score_based'] = (scoresSBM_search_m_j['score_perc'] >= score_cut_off)
...
def perform_full_nn_match(ref, ref_address_cols, search_df, search_address_cols,
                      ...
                      standardise, exported_model, matching_variables,
                       text_columns, weights, fuzzy_method, score_cut_off,
                         match_results, filter_to_lambeth_pcodes, new_join_col=["UPRN"]):
    ...
    list_out, predict_df = full_predict_func(predict_data, exported_model)

    toc = time.perf_counter()

    print(f"Performed the NN prediction in {toc - tic:0.1f} seconds")

    ### Fill in any columns that are missing and reorder

    predict_df = post_predict_clean(predict_df = predict_df, orig_search_df =search_df_prep, ref_address_cols = ref_address_cols,
    ...
    all_scores_pc, scoresSBM_out_pc, scoresSBM_best_pc, matched_output_SBM_pc = score_based_match(predict_df_search = predict_df_search.copy(), ref_search = ref_search,
    ...
    all_scores_st, scoresSBM_out_st, scoresSBM_best_st, matched_output_SBM_st = score_based_match(predict_df_search = predict_df_search.copy(), ref_search = ref_search, 
                                                                                                  orig_search_df = search_df_prep, matching_variables = matching_variables,
                      text_columns = text_columns, blocker_column = blocker_column, weights = weights, fuzzy_method = fuzzy_method, score_cut_off = score_cut_off,
                                               search_df_key_field=search_df_key_field, standardise=standardise, new_join_col=new_join_col)
    ...
    if type(matched_output_SBM_st) == int:
